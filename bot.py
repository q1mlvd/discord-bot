...
class Database:
    ...
    async def init_db(self):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            # Таблица заказов (ПЕРЕСОЗДАЕМ ПРАВИЛЬНО)
            await db.execute('DROP TABLE IF EXISTS orders')  # Удаляем старую таблицу
            await db.execute('''
                CREATE TABLE orders (
...
async def reset_weekly_xp(self):
    ...
    async def add_xp(self, user_id: int, xp_gain: int):
        ...
        async with aiosqlite.connect(self.db.db_path) as db:
            ...
            # Добавляем XP и проверяем уровень
            ...
            async with db.execute('SELECT xp, level FROM users WHERE user_id = ?', (user_id,)) as cursor:
                ...
                if user_data:
                    ...
                    if xp >= xp_needed:
                        # Повышаем уровень и сбрасываем XP
...
class ShopSystem:
    ...
    async def create_order(self, user_id: int, item_id: int, quantity: int = 1, details: str = ""):
        ...
        if product.get("per_unit"):
            ...
            quantity = 1  # Для товаров не по единицам количество всегда 1
...
async def weekly_reset_task(self):
    ...
    while not self.is_closed():
        now = datetime.now()
        # Сбрасываем каждый понедельник в 00:00
...
async def работа(interaction: discord.Interaction):
    try:
        ...
        if user_data["work_cooldown"]:
            ...
            if (datetime.now() - last_work).seconds < 600:  # 10 минут
...
async def категория(interaction: discord.Interaction, название: str):
    # Сопоставление упрощенных названий с реальными
    category_map = {
        "tds": "🎮 TDS/TDX",
        "tdx": "🎮 TDS/TDX", 
        "roblox": "🔴 Roblox",
    ...
    if название not in bot.shop.categories:
        ...
        await interaction.response.send_message(
            f"❌ Категория не найдена!\n\n**Доступные категории:**\n{available_categories}\n\n**Используйте:**\n`/категория название: TDS/TDX`\n`/категория название: Roblox`\n`/категория название: Blox Fruits`\n`/категория название: Discord`", 
            ephemeral=True
        )
        return
...
async def купить(interaction: discord.Interaction, id_товара: int, количество: int = 1, детали: str = ""):
    # Проверка минимального количества для Robux
...
async def уровень(interaction: discord.Interaction, пользователь: Optional[discord.Member] = None):
    ...
    embed = Design.create_embed("🏆 Уровень", 
                              ...
                              f"Уровень: {level}\n"
                              f"Опыт: {xp}/{xp_needed}\n"
                              f"Прогресс: {xp}/{xp_needed}", "primary")
    await interaction.response.send_message(embed=embed)
...
async def очистить(interaction: discord.Interaction, количество: int):
    try:
        ...
        deleted = await interaction.channel.purge(limit=количество + 1)  # +1 для команды
...
async def on_ready():
    ...
    await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="/помощь"))
...
async def on_message(message):
    ...
    # Опыт за сообщения (только в текстовых каналах)
